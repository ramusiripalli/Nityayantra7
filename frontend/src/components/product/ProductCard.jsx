...
export const ProductCard = ({ product, isVideoCard = false, className = '' }) => {
  ...
  return (
    <div className={`group flex flex-col bg-white rounded-[14px] border border-slate-200 shadow-2xs hover:shadow-md hover:-translate-y-[3px] transition-all duration-200 overflow-hidden text-left h-full w-full justify-between ${className}`}>
      ...
      <div className="relative w-full aspect-square bg-[#f8fafc] border-b border-slate-100 flex items-center justify-center p-3.5 sm:p-4 shrink-0 overflow-hidden">
        <img
          src={product.image}
          alt={product.title}
        ...
        {/* Top-Left Compact Green Discount Badge */}
        {discount > 0 && !isVideoCard && (
          <span className="absolute top-2.5 left-2.5 bg-emerald-600 text-white font-extrabold text-[10px] sm:text-[11px] px-2 py-0.5 rounded-md shadow-2xs tracking-wide">
            {discount}% OFF
          </span>
        )}
      ...
      {/* 2. PRODUCT INFORMATION AREA (Consistent Visual Hierarchy) */}
      <div className="flex flex-col flex-1 p-3 justify-between space-y-2">
        
        <div className="space-y-1.5">
          {/* Price Hierarchy (Current Price Dominant + Crossed Original + Savings) */}
          ...
          {/* Product Title (14px Dark Font, Strict 2-Line Clamping) */}
          <Link to={`/product/${product.id}`} className="block group-hover:text-sky-600 transition-colors">
            <h3 className="text-xs sm:text-[14px] font-semibold text-slate-900 line-clamp-2 h-[2.3rem] sm:h-[2.5rem] leading-[1.35]">
              {product.title}
            </h3>
          </Link>

          {/* Unit / Pack Size Metadata */}
        ...
        <div className="space-y-1.5 pt-1.5 border-t border-slate-100 mt-auto">
          ...
          {/* Primary Action Button: "Compare Deals →" */}
          <Link
            to={`/product/${product.id}`}
            className="w-full h-[30px] bg-sky-50 hover:bg-sky-600 text-sky-700 hover:text-white font-bold text-[11px] rounded-lg border border-sky-200/80 hover:border-sky-600 transition-all duration-200 flex items-center justify-center gap-1 mt-1 cursor-pointer shadow-2xs"
          >
